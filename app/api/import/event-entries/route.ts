...
export async function POST(request: Request) {
  ...
  if (headers.some((header) => !header)) {
    return NextResponse.json({ error: "CSV header row contains empty column names." }, { status: 400 });
  }

  const historyIndex = headers.indexOf("history_id");
  if (historyIndex === -1) {
    return NextResponse.json({ error: "CSV must include a history_id column." }, { status: 400 });
  }
  ...
  rows.slice(1).forEach((row, rowIndex) => {
    ...
    if (historyId === null) {
      rowErrors.push(`Row ${rowIndex + 2}: missing or invalid history_id.`);
      return;
    }
  ...
  records.forEach((record) => {
    ...
    if (typeof historyId === "number") {
      if (deduped.has(historyId)) duplicateCount += 1;
      deduped.set(historyId, record);
    }
  });

  const dedupedRecords = Array.from(deduped.values());
  if (duplicateCount > 0) {
    errors.push(`Skipped ${duplicateCount} duplicate history_id rows (kept last).`);
  }
  ...
  for (let start = 0; start < dedupedRecords.length; start += MAX_BATCH_SIZE) {
    const batch = dedupedRecords.slice(start, start + MAX_BATCH_SIZE);
    const { error } = await supabase
      .from("event_entries")
      .upsert(batch, { onConflict: "history_id" });
